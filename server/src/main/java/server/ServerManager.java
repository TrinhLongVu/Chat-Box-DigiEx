...
public class ServerManager {
    ...
    public void startServer(int port) {
        this.PORT = port;
        running = true;
        Logger.getLogger(ServerManager.class.getName()).log(Level.INFO, "Server is running....");
        new Thread(() -> {
            try {
    ...
    private class HeartbeatSender implements Runnable {
        ...
        public void run() {
            while (true) {
                try {
                    Send send = new Send(brokerSocket);
                    send.sendData("type:heartbeat");
